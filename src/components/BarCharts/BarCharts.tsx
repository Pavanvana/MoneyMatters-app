...
const BarCharts = () => {
  ...
  const [last7DaysCreditsAndDebitsDate, setLast7DaysCreditsAndDebitsDate] =
    useState<Array<ResponseData>>([]);
  const url =
    userId === "3"
  ...
  const getLast7daysCreditsAndDebits = async () => {
    ...
    if (dataOf7Days !== null) {
      if (userId === "3") {
        setLast7DaysCreditsAndDebitsDate(
          dataOf7Days.last_7_days_transactions_totals_admin
        );
      } else {
        setLast7DaysCreditsAndDebitsDate(
          dataOf7Days.last_7_days_transactions_credit_debit_totals
        );
      }
    }
  };
  ...
  const renderBarchart = () => {
    ...
    for (let i = 0; i < 7; i++) {
      const findData =
        last7DaysCreditsAndDebitsDate !== undefined &&
        last7DaysCreditsAndDebitsDate.filter(
          (each) => new Date(each.date).getDay() === i
        );
      let creditSum;
      let debitSum;
      if (findData !== false) {
        const credit = findData.find(
          (each: ResponseData) => each.type === "credit"
        );
        const debit = findData.find(
          (each: ResponseData) => each.type === "debit"
        );
  ...
  return (
    <div className="debit-credit-overview-container">{renderOnApiStatus()}</div>
  );
};
